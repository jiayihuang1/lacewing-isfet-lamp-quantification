"""B4 V1 — Fine-tune the Track C winner on UTI data only.

Loads the a3+alpha0.5+contrastive_epochs100 winner best.pt (COV-trained),
then fine-tunes the joint model on UTI amp-positive pixels ONLY (no NTC
augmentation, no COVID data), leave-one-UTI-chip-out.
...
Rationale for LR = 1e-4 (10× lower than Track C's initial 1e-3): full
fine-tuning small dataset (~14k UTI pixels) — a small LR keeps the encoder
close to the COV-trained representation while letting the joint model
adapt to the UTI signal statistics.

Output dir:
    Analysis/quantification/methods/results/b4_finetune_uti_from_a3_ep100/
        seed{S}_holdout_{chip_tag}/
            config.json / history.json / predictions.npz / labels.npz / checkpoints/

Usage:
    python -m lacewing.quantification.methods.b4_finetune_uti \\
        --seed 0 --holdout-chip uti_260728_EC_SD --epochs 10
"""
...
COV_WINNER_CKPT = LACEWING_PKG_DIR / "quantification" / "methods" / "results" / "p3_a3_alpha0.5_unet_w120_stride10_ptcontrastive_epochs100" / "seed0" / "checkpoints" / "best.pt"
RESULTS_ROOT = LACEWING_PKG_DIR / "quantification" / "methods" / "results" / "b4_finetune_uti_from_a3_ep100"

# Joint-model config MUST match the winner checkpoint's config exactly.
ARCH = "a3"
ALPHA = 0.5
WINDOW = 120
STRIDE = 10
...
def main() -> None:
    ...
    # Build joint model + load COV-trained winner.
    model = build_joint_model(arch=ARCH, window=WINDOW, alpha=ALPHA).to(args.device)
    print(f"[fine-tune] loading COV winner from {COV_WINNER_CKPT}")
    ckpt = torch.load(COV_WINNER_CKPT, map_location=args.device, weights_only=False)
    ...
    (out_dir / "config.json").write_text(json.dumps({
        ...
        "pretrained_from": str(COV_WINNER_CKPT.relative_to(PROJECT_ROOT)),
