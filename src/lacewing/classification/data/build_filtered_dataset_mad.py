"""Build a per-pixel dataset cache with the MAD-based four-layer
chip-relative filter applied per chip.

Sibling of ``build_filtered_dataset.py``; the only difference is the
threshold rule (Layers A, B, D use ``median +- k * MAD`` instead of
percentile cuts).  Layer C is unchanged.

Caller specifies:
  - ``scope``  ('final' or 'all')
  - ``layers`` ('A' / 'AB' / 'ABC' / 'ABCD')
  - ``clean_ntc_first`` (bool)
  - ``k`` (the MAD multiplier)
  - ``out_name`` cache stem
...
def _filter_one_chip(
    ...
    keep_mask, info = filter_core.run_filter_pipeline_per_chip_mad(
    ...
    log_lines.append(
        f"    filter {layers}/ntc={'D' if clean_ntc_first else 'raw'}/k={k}:"
...
def build_filtered_mad(
    ...
    log_lines = [
        f"# build_filtered_dataset_mad.py run at {datetime.now().isoformat()}",
        f"# scope={scope}, layers={layers}, clean_ntc_first={clean_ntc_first}, k={k}",
...
def default_out_name_mad(scope: str, layers: str, clean_ntc_first: bool,
                         k: float) -> str:
...
def main() -> None:
    ...
    out_name = args.out_name or default_out_name_mad(
        args.scope, args.layers, args.clean_ntc, args.k)
    build_filtered_mad(
        scope=args.scope,
        layers=args.layers,
